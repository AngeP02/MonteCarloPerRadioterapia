...
__device__ inline void metodo_kahn(double energia_mev, double xi_1, double xi_2, double xi_3, double &cos_theta, double &energia_scatter)
{
    ...
    if (xi_1 * area_totale < area_ramo_1) {
        ...
        tau = sqrt(fmax(tau_quadro, 1e-30));
    }
    ...
    probabilita_accettazione = fmax(0.0, fmin(probabilita_accettazione, 1.0));

    if (xi_3 > probabilita_accettazione)
        cos_theta = 2.0;
}

__device__ inline void aggiornamento_traiettoria( double &ux, double &uy, double &uz, double cos_theta, double phi)
{
    ...
    if (fabs(uz) > 0.99999) {
        ...
        double proiezione_xy = sqrt(1.0 - uz * uz);
        ux_new = sin_theta * (ux * uz * cos_phi - uy * sin_phi) / proiezione_xy + ux * cos_theta;
        uy_new = sin_theta * (uy * uz * cos_phi + ux * sin_phi) / proiezione_xy + uy * cos_theta;
