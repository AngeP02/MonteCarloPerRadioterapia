...
// INDICI MATERIALI
#define MATERIALE_ACQUA 0
#define MATERIALE_OSSO 1
#define NUMERO_MATERIALI 2

__constant__ double DENSITA[NUMERO_MATERIALI] = { 1.000, 1.850 };

static const int PUNTI_CAMPIONAMENTO = 28;

__constant__ double GRIGLIA_ENERGIA[PUNTI_CAMPIONAMENTO] = {
...
__constant__ double PROBABILITA_TOTALE[NUMERO_MATERIALI][PUNTI_CAMPIONAMENTO] = {
...
__constant__ double PROBABILITA_FOTOELETTRICO[NUMERO_MATERIALI][PUNTI_CAMPIONAMENTO] = {
...
__constant__ double PROBABILITA_COMPTON[NUMERO_MATERIALI][PUNTI_CAMPIONAMENTO] = {
...
__constant__ double PROBABILITA_COPPIA[NUMERO_MATERIALI][PUNTI_CAMPIONAMENTO] = {
...
__constant__ double ENERGIE_SPETTRO[NUMERO_BINS_SPETTRO] = {
    0.25, 0.50, 0.75, 1.00, 1.25, 1.50, 1.75, 2.00,
    2.25, 2.50, 2.75, 3.00, 3.25, 3.50, 3.75, 4.00,
    4.25, 4.50, 4.75, 5.00, 5.25, 5.50, 5.75, 6.00
};

__constant__ double FLUENZA_SPETTRO[NUMERO_BINS_SPETTRO] = {
...
__device__ inline double interpolazione_lineare_energia(double energia_mev, const double *tabella) {
    ...
    int indice_limite_superiore = PUNTI_CAMPIONAMENTO - 1;

    while (indice_limite_superiore - indice_limite_inferiore > 1) {
    ...
    double fattore_interpolazione = (energia_mev - GRIGLIA_ENERGIA[indice_limite_inferiore]) / (GRIGLIA_ENERGIA[indice_limite_superiore] - GRIGLIA_ENERGIA[indice_limite_inferiore]);
    return tabella[indice_limite_inferiore] * (1.0 - fattore_interpolazione) + tabella[indice_limite_superiore] * fattore_interpolazione;
}

__device__ inline double calcolo_attenuazione_totale(double energia, int materiale) {
    return interpolazione_lineare_energia(energia, PROBABILITA_TOTALE[materiale]) * DENSITA[materiale];
}
__device__ inline double calcolo_attenuazione_fotoelettrico(double energia, int materiale) {
    return interpolazione_lineare_energia(energia, PROBABILITA_FOTOELETTRICO[materiale]) * DENSITA[materiale];
}
__device__ inline double calcolo_attenuazione_compton(double energia, int materiale) {
    return interpolazione_lineare_energia(energia, PROBABILITA_COMPTON[materiale]) * DENSITA[materiale];
}
__device__ inline double calcolo_attenuazione_coppie(double energia, int materiale) {
    return interpolazione_lineare_energia(energia, PROBABILITA_COPPIA[materiale]) * DENSITA[materiale];
}

__device__ inline int seleziona_tipo_interazione(double energia, int materiale, float xi) {
    ...
    if ((double)xi < probabilita_fotoelettrico)
        return 0;
    if ((double)xi < probabilita_fotoelettrico + probabilita_compton)
        return 1;
    return 2;
}

__host__ __device__ inline int phantom_idx(int ix, int iy, int iz) {
    return ix + NX * (iy + NY * iz);
}

__host__ __device__ inline bool verifica_confini(double x, double y, double z) {
    return x >= 0.0 && x < PHANTOM_CM &&
           y >= 0.0 && y < PHANTOM_CM &&
           z >= 0.0 && z < PHANTOM_CM;
}

__host__ __device__ inline int vox(double coord) {
