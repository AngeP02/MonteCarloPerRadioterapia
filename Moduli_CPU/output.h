...
inline void calcolo_pdd(const double *dose, double *pdd, double *profondita, int semiampiezza_della_media = 8) {
...
inline void calcolo_profilo_laterale(const double *dose, double *profilo_dose_relativa, double *coordinate_cm, double profondita_scelta = 10.0, int semiampiezza_media = 2) {
...
inline void salva_pdd_csv(const double *vettore_profondita_cm, const double *pdd, const char *filename) {
...
inline void salva_profilo_csv(const double *coordinate_cm, const double *profilo_dose_relativa, const char *filename) {
    std::ofstream f(filename);
    f << " ,dose_percent\n";
    for (int ix = 0; ix < NX; ix++)
...
inline void salva_fetta_dose_csv(const double *dose, const char *filename) {
    std::ofstream f(filename);
    int iy = NY / 2;
    for (int iz = 0; iz < NZ; iz++) {
        for (int ix = 0; ix < NX; ix++) {
...
inline void salva_volume_completo(const double *dose, const char *filename) {
...
inline void stampa_tabella_pdd(const double *profondita, const double *pdd, const char *label) {
    printf("\n  PDD — %s\n", label);
...
inline void stampa_statistiche_dose(const double *dose, long long numero_particelle_totali, double tempo_esecuzione_secondi) {
