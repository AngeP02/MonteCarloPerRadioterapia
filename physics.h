 ...
 * physics.h
 * ─────────────────────────────────────────────────────────────────────────────
 * Costanti fisiche e tabelle dei coefficienti di attenuazione da NIST XCOM.
 * Ogni valore è documentato con fonte e unità.
 *
 * Fonte coefficienti:
 *   Hubbell J.H., Seltzer S.M. (1996) - NIST Technical Report
...
static const double PCUT     = 0.100;       // MeV  cutoff elettroni (non usato: deposito locale)
...
// INDICI MATERIALI
// ─────────────────────────────────────────────────────────────────────────────
#define MAT_WATER 0   // acqua  ρ = 1.000 g/cm³
#define MAT_BONE  1   // osso corticale ICRU  ρ = 1.850 g/cm³
#define MAT_LUNG  2   // polmone ICRU  ρ = 0.260 g/cm³
#define MAT_AIR   3   // aria secca  ρ = 0.001205 g/cm³
#define N_MAT     4
...
// Fonte: NIST XCOM tabella standard
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// COEFFICIENTI μ/ρ [cm²/g]  per ogni materiale e processo
// Layout: [materiale][bin_energia]
// ─────────────────────────────────────────────────────────────────────────────

// ── μ_total / ρ ──────────────────────────────────────────────────────────────
static const double MU_TOTAL[N_MAT][N_ENERGY] = {
    // ACQUA  H₂O  ρ=1.000 g/cm³
    { 5.329, 1.673, 0.8096, 0.3756, 0.2683, 0.2269, 0.2059, 0.1837, 0.1707,
      ...
      0.01941, 0.01813 },
    // OSSO CORTICALE ICRU  ρ=1.850 g/cm³
    { 19.89, 7.131, 3.085, 1.012, 0.5475, 0.3941, 0.3178, 0.2595, 0.2368,
      ...
      0.01978, 0.01832 },
    // POLMONE ICRU  ρ=0.260 g/cm³  (≈ tessuto molle, densità ridotta)
...
// ── μ_produzione_coppie / ρ  (zero sotto 1.022 MeV) ─────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
inline double interp_mu(double energy_mev, const double table[N_ENERGY]) {
    // Clamp agli estremi
    if (energy_mev <= ENERGY_GRID[0])         return table[0];
    if (energy_mev >= ENERGY_GRID[N_ENERGY-1]) return table[N_ENERGY-1];

    // Ricerca binaria del bin
    int lo = 0, hi = N_ENERGY - 1;
    while (hi - lo > 1) {
        int mid = (lo + hi) / 2;
        if (ENERGY_GRID[mid] <= energy_mev) lo = mid;
        else                                hi = mid;
    }
    // Interpolazione lineare
    double t = (energy_mev - ENERGY_GRID[lo]) /
               (ENERGY_GRID[hi] - ENERGY_GRID[lo]);
    return table[lo] * (1.0 - t) + table[hi] * t;
}

// ─────────────────────────────────────────────────────────────────────────────
// INTERFACCE PUBBLICHE μ [cm⁻¹]  =  (μ/ρ) × ρ
// ─────────────────────────────────────────────────────────────────────────────
inline double get_mu_total(double E, int mat) {
    return interp_mu(E, MU_TOTAL[mat]) * DENSITY[mat];
}
inline double get_mu_pe(double E, int mat) {
    return interp_mu(E, MU_PE[mat]) * DENSITY[mat];
}
inline double get_mu_compton(double E, int mat) {
    return interp_mu(E, MU_COMPTON[mat]) * DENSITY[mat];
}
inline double get_mu_pair(double E, int mat) {
    return interp_mu(E, MU_PAIR[mat]) * DENSITY[mat];
}
...
// ─────────────────────────────────────────────────────────────────────────────
inline int select_interaction(double E, int mat, double xi) {
    double mu_tot = get_mu_total(E, mat);
    if (mu_tot <= 0.0) return 1;  // fallback Compton

    double p_pe  = get_mu_pe(E, mat)      / mu_tot;
    double p_cmp = get_mu_compton(E, mat) / mu_tot;
    // p_pair = 1 - p_pe - p_cmp

    if (xi < p_pe)             return 0;   // fotoelettrico
    if (xi < p_pe + p_cmp)    return 1;   // Compton
    return 2;                              // produzione coppie
}

// ─────────────────────────────────────────────────────────────────────────────
// INDICE LINEARE PHANTOM  (row-major: X varia più veloce)
// ─────────────────────────────────────────────────────────────────────────────
